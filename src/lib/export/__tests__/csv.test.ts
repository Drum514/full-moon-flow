...
function makeEntry(date: string, intensity: CycleEntry['flowIntensity'] = 'medium'): CycleEntry {
  return {
    id: `id-${date}`,
    date,
...
describe('CSV Export', () => {
  it('returns just the header for an empty array', () => {
    expect(generateCSV([])).toBe('date,flow_intensity');
  });
  ...
  it('generates correct CSV format for entries', () => {
    ...
    expect(lines[0]).toBe('date,flow_intensity');
